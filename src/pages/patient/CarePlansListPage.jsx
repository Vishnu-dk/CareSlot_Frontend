...
import { useNavigate } from "react-router-dom"; // Optional: If you want URL params
import { Box, Flex, Heading, Text, VStack, Spinner, SimpleGrid, Button } from "@chakra-ui/react";
...
import CarePlanDetailModal from "../../components/patient/CarePlanDetailModal"; // ✅ Import Detail Modal
  ...
  if (error || plans.length === 0) {
    return (
      <Box maxW="900px" mx="auto" py={4}>
        <Box bg="beige" borderRadius="20px" p={14} boxShadow={CARD_SHADOW} textAlign="center">
        ...
        {plans.map((plan) => {
          ...
          return (
            <Box
              ...
              _hover={{ transform: "translateY(-2px)", boxShadow: "0 8px 24px rgba(62,39,35,0.1)" }}
              onClick={() => setSelectedPlanId(plan.id)} 
            >
              <Flex justify="space-between" align="start" mb={4}>
                <Box flex={1} pr={4}>
                  <Text fontSize={{base: "16px", md: "18px"}} fontWeight={700} color="espresso" lineClamp={1}>
                    {plan.title}
                  </Text>
                  <Text fontSize={{base: "11px", md: "13px"}} color="#5D4037" mt={1}>
                    Assigned by {plan.clinicianName}
                  </Text>
                  <Text fontSize={{base: "9px", md: "11px"}} color="#5D4037" opacity={0.7} mt={1}>
                ...
                <Box position="relative" w= "50px" h="50px" flexShrink={0}>
                   ...
                   <Flex position="absolute" inset={0} align="center" justify="center">
                     <Text fontSize={{base: "8px", md: "10px"}} fontWeight={800} color="espresso">{progress}%</Text>
                   </Flex>
                </Box>
              </Flex>

              <Flex justify="space-between" align="center" pt={3} borderTopWidth="1px" borderColor="taupe">
                 <StatusBadge status={plan.status} />
                 <Flex align="center" gap={1} color="brand.500" fontWeight={600} fontSize={{base: "11px", md: "13px"}}>
                   <Text>{doneCount}/{plan.tasks.length} Tasks</Text>
