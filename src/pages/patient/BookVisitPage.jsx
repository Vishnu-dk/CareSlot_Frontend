...
export default function BookVisitPage() {
  ...
  const { data: weekly = [] } = useGetWeeklyAvailabilityQuery(
    selected?.userId,
    { skip: !selected },
  );

  const { data: rawSlots = [], isFetching } = useGetAvailableSlotsQuery(
  ...
  const availableSlots = useMemo(() => {
    ...
    return rawSlots.filter((slot) => {
      const startIso = combineToISOWithOffset(selectedDate, slot.startTime);
      return isFutureSlot(startIso);
    });
  ...
  const handleConfirmBooking = async ({ reason }) => {
    setModalError(null);
    try {
      const startUtc = localToUtcTime(selectedDate, bookingSlot.startTime);
      const endUtc = localToUtcTime(selectedDate, bookingSlot.endTime);
            ...
            {clinicians.map((c) => (
              <Box
                ...
                _hover={{
                  ...
                  boxShadow: "0 8px 28px rgba(62,39,35,0.12)",
                }}
                onClick={() => {
                  setSelected(c);
                  setSelectedDay(null);
    ...
    <Box maxW="900px" mx="auto" py={4}>
      ...
      >
        ...
        <Box flex={1}>
          ...
          <Text color="brand.500" fontWeight={600} fontSize="14px" mb={1.5}>
            {selected.specialty}
          </Text>
          <Text fontSize="13px" color="#5D4037">
            Pick a day below, then choose a time slot.
          </Text>
        </Box>
      </Flex>
      ...
      <Box bg="beige" borderRadius="20px" p={6} boxShadow={CARD_SHADOW} mb={4}>
        <Text fontSize="18px" fontWeight={700} color="espresso" mb={4}>
          Weekly Availability
        </Text>
          ...
          {DAY_NAMES.map((day, i) => {
            ...
            return (
              <Box key={day} flex="1" minW="72px" textAlign="center">
                <Text fontSize="11px" fontWeight={600} color="#5D4037" mb={2}>
                  {day}
                </Text>
                <Button
                  w="100%"
                  ...
                  _hover={{
                    bg: sel
                      ? "brand.600"
                      : av
                        ? "rgba(160,120,85,0.18)"
                        : "rgba(215,204,200,0.3)",
                  }}
                  onClick={() => {
                    setSelectedDay(sel ? null : i + 1);
                    setSelectedDate(null);
                  }}
                >
                  {av
      ...
      {selectedDay && (
        ...
        >
          <Text fontSize="18px" fontWeight={700} color="espresso" mb={4}>
            Choose a Date — {DAY_NAMES[selectedDay - 1]}
          </Text>
          <Flex gap={2} flexWrap="wrap">
            {nextDatesForWeekday(selectedDay).map((iso) => (
              <Button
                ...
                border={selectedDate === iso ? "none" : "1.5px solid"}
                borderColor="brand.500"
                _hover={{
                  bg:
                    selectedDate === iso ? "brand.600" : "rgba(160,120,85,0.1)",
                }}
              >
                {fmtDate(iso)}
              </Button>
      ...
      {selectedDate && (
        <Box bg="beige" borderRadius="20px" p={6} boxShadow={CARD_SHADOW}>
          ...
          {isFetching ? (
            ...
            <VStack spacing={2} py={6} align="center">
              <Text fontSize="14px" color="#5D4037" fontWeight={600}>
                No open slots remaining for this day.
              </Text>
              <Text fontSize="12px" color="#5D4037">
