...
export default function AdminDashboard() {
  const { data: clinicians = [], isLoading: loadClin } =
    useGetAllCliniciansQuery();
  const { data: patients = [], isLoading: loadPat } = useGetAllPatientsQuery();
  const { data: appointments = [], isLoading: loadAppt } =
    useGetAllAppointmentsQuery();
  ...
  const stats = useMemo(() => {
    ...
    const completionRate =
      totalAppts > 0 ? ((completedAppts / totalAppts) * 100).toFixed(1) : 0;
            ...
            {recentActivity.length === 0 ? (
              ...
              recentActivity.map((item, i) => (
                <Flex
                  ...
                  py={3}
                  borderBottomWidth={
                    i < recentActivity.length - 1 ? "1px" : "none"
                  }
                ...
                >
                  <Box
                    ...
                    h="8px"
                    borderRadius="full"
                    bg={item.status === "COMPLETED" ? "#15803d" : "#A07855"}
                    flexShrink={0}
                  />
                  <Box flex={1}>
                    <Text
          ...
          <VStack spacing={6} align="stretch">
            <Box>
              <Flex
                ...
                mb={2}
              >
                <Text fontWeight={500} color="espresso">
                  Patients
                </Text>
