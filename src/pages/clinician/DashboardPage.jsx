...
import { format, addDays, parseISO, isWithinInterval, subMinutes, addMinutes } from "date-fns";
import {
  Box, Button, Flex, Grid, Heading, HStack, Text, VStack, Spinner, Tooltip,
...
import {
  ...
  useGetMyPatientsPlansQuery, // ✅ NEW HOOK: Fetches all plans issued by this doctor
...
export default function ClinicianDashboard() {
  ...
  // 1. Fetch Schedule
  const { data: schedule = [], isLoading: loadingSchedule } = useGetMyScheduleQuery(
    { date: selectedDate },
  ...
  const hasPlanForPatient = (patientId) => {
    return myPlans.some(plan => plan.patientId === patientId && plan.status !== "CANCELLED");
  };

  const isPlanWindowOpen = (appointment) => {
    if (!appointment || !appointment.startsAt || !appointment.endsAt) return false;
    const now = new Date();
    ...
    <Box mx="auto" py={4}>
      ...
      <Box bg="beige" borderRadius="20px" p={6} boxShadow={CARD_SHADOW} mb={6}>
        ...
        {loadingSchedule ? (
          ...
          <Text color="#5D4037" textAlign="center" fontSize={{base: "12px", md: "15px"}} py={8}>No appointments scheduled.</Text>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <Flex align="center" gap={3} flex={1}>
                    ...
                    >
                      {(appointment.patientName || "PA").slice(0, 2).toUpperCase()}
                    </Flex>
                    <Box>
                      ...
                      {!isDone && !patientHasPlan && (
                         <Text fontSize="10px" color="#dc2626" fontWeight={600} mt={1}>
      ...
      <Grid templateColumns={{ base: "repeat(1, 1fr)", md: "repeat(2, 1fr)" }} gap={4}>
        {[
          ...
          { label: "Pending Plans", value: `${schedule.filter(a => a.status === 'BOOKED' && !hasPlanForPatient(a.patientId)).length}`, sub: "need care plans" },
        ].map((stat) => (
